...
Rare but serious side effects included: the non-technical cofounder requesting GitHub admin access, forming opinions about CI/CD pipelines, referring to the test suite as "my tests," explaining stale closures to the engineering team despite not knowing what a closure is, and in one case, mass Slacking a screenshot of a green test suite at 4am with the caption "we're so back."

In the latest formulation, additional side effects have been observed: unsolicited opinions about React hook dependency arrays, a compulsive need to check whether other people's PRs conflict with theirs, and in one case, a cofounder who refused to merge a PR until "the loop passes clean on all three iterations" — a phrase they could not define when questioned.
